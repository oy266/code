    ...
    def evaluate(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                outputs = model(imgs) # outputs应该是多帧的结果 [0(帧号): fpn_outs, 1: , 2:, ...]
            ...
            for i in range(self.num_frames): # 对每一个image使用convert_to_coco_format
                data_list_elem, image_wise_data = self.convert_to_coco_format(
                    outputs[i], info_imgs[:, :, i], ids[:, i], return_outputs=True)
