...
class VOCDetection(CacheDataset):
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self.root, "results") #(self.root, "results", "VOC" + self._year, "Main")
    ...
    def _do_python_eval(self, output_dir="output", iou=0.5):
        rootpath = self.root # os.path.join(self.root, "VOC" + self._year)
        #name = self.image_set[0][1]
        name = self.image_set[0]
        annopath = os.path.join(rootpath, "Annotations", "{}.xml")
        imagesetfile = os.path.join(rootpath, "ImageSets", "Main", name + ".txt")
        cachedir = os.path.join(
            self.root, "annotations_cache") # self.root, "annotations_cache", "VOC" + self._year, name
